...
public class NiftyHTTP {
	...
	public static void main(String[] args){
		...
		hm.put("pub_open", "1");
		hm.put("food_ready", "0");
		hm.put("food_description", "kött");
		hm.put("event", "KÖTT FÖ FÅÄÄEEEEEN");
		
		System.out.print(nh.postAnnouncement(hm));
	}
	...
	public String postAnnouncement(HashMap<String, String> announcements) {
		...
		/*
		 * Typically, these announcements can be posted:
		 * pub_open ( = 0 or 1), food_ready ( = 0 or 1), food_description, event
		 */
	...
	private boolean validateToken() {
		...
		try {
			params.add(new BasicNameValuePair("username", URLEncoder.encode(this.username, ENCODING)));
			params.add(new BasicNameValuePair("token", URLEncoder.encode(this.authToken, ENCODING)));
