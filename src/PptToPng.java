...
public class PptToPng {
	 ...
	 * @param tmpDims Rectangle which width and height is the the dimensions the slides should be sclaed to.
	 */
	 ...
	 * @param tmpPath constructor which sets the filepath to the passed argument.
	 * @param tmpDims Rectangle which width and height is the the dimensions the slides should be sclaed to
	 */
	 ...
	 * @param tmpH int, height which the slide should be scaled to.
	 */
	 ...
	 * @param tmpW int, width which the slide should be scaled to.
	 */
	 ...
	 * @param tmp Rectangle, containing the width and height which the slides should be scaled to
	 */
	 ...
	 * @param tmpPath	String, Filepath.
	 * @param tmpDims	Rectangle, containing the dimensions for scaling.
	 */
	...
	/**
	 * Method for extracting all of the slides in a .ppt file to png format stored in .hansimage files.
	 * 
	 */
	public void extract() {
...
/**
 * Method for extracting the slides with the passed parameters without setting the objects parameters.
 * @param tmpPath	String, filepath.
 * @param tmpDims	Rectangle, Dimension which the slides should be scaled to.
 */
	 ...
	 *Method for extracting the slides with the passed parameters without setting the objects parameters.
	 * @param tmpPath	String, filepath.
	 * @param tmpwidth int, width which the slides should be scaled to.
	 * @param tmpHeight int, height which the slides should be scaled to.
	 */
	 ...
	 * @param tmpFilepath Path to the .ppt file.
	 ...
	 * @param tmpImg	BufferedImage, Image to save as file.
	 * @param tmpName	String, the filename.
	 * @throws IOException
	 */
	...
	private void converter(String tmpFilepath, int tmpimgWidth, int tmpImgHeight){
		...
		for (int i = 0; i < slide.length; i++) {
			...
			//if tmpimgWidth is 0 the slide shouldn't be scaled and therefore is slideToImage called with the scaling values set to slide size.
			if(tmpimgWidth==0){
				 bImg = slideToImage(slide[i],dimension,dimension.width,dimension.height);
			}else{
				 bImg = slideToImage(slide[i],dimension,tmpimgWidth,tmpImgHeight);
			}
	 ...
	 */
	private BufferedImage slideToImage(Slide tmpslide, Dimension dimension, int tmpimgWidth,int tmpImgHeight){
		
		BufferedImage img = new BufferedImage(tmpimgWidth, tmpImgHeight,BufferedImage.TYPE_INT_RGB);
		
		//Creates and calculate the scaling properties
        AffineTransform at = new AffineTransform();
        at.scale((tmpimgWidth/dimension.getWidth()),(tmpImgHeight/dimension.getHeight()));
