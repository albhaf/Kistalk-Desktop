...
public class ShowImage extends JPanel{
    ...
    //Variabler som s�tts i konstruktorn
	private Rectangle monitorSize;
	private Dimension textStartPosition =new Dimension(); // width == Ypos, height == Xpos.
	private int correction;
	private BufferedImage image;
	private Dimension timeStill = new Dimension(); // height == time to stand still, width == time for current image
	private float imageStopPosition;
	private Dimension imageSize = new Dimension(); // width == image size x, height == image size y.
	...
	public ShowImage(BufferedImage tmp, String tmpImageUserString,String tmpImageCommentString, Rectangle tmpmonitor, int tmpTimeStill ,List<CommentXML> ImageComments) {

		monitorSize = tmpmonitor;
		
		float factor = (float)(tmp.getWidth()) / (float)(tmp.getHeight());
		
	    image = new BufferedImage(100, /*(int)(100*factor)*/100, BufferedImage.TYPE_INT_RGB);
...
//	    imageComments = new TextDisplay[ImageComments.size()];
	//    for(int i =0;i<imageComments.length;i++){
	  ...
	  //  	System.out.println(ImageComments.get(i).getContent());
	    ...
	    //ImageUser setup
	    textStartPosition.width = (monitorSize.width/2)-(correction*(ImageUserFontSize/2));
        ...
        Rect = new ImgRect(-200,100,imageSize.width*factor,imageSize.height);
    }
	...
	public void scalePicturesText(){
	    ...
	    textStartPosition.width = (monitorSize.width/2)-(correction*(ImageUserFontSize/3));
	}
	
	public void UpdatePicture(BufferedImage tmp, String tmpUserString,String tmpImageText,List<CommentXML> ImageComments){
        //Bilden
		slideImage = tmp;
        Rect.resetPos();
        //Rect.addHeight(h);
	  ...
	  //  for(int i =0;i<imageComments.length;i++){
       ...
       imageComments = new TextDisplay[ImageComments.size()];
        for(int i =0;i<imageComments.length;i++){
        String bla =(ImageComments.get(i).getUser() + " wrote: " +ImageComments.get(i).getContent());
    ...
    public void paint(Graphics g) {
        ...
        if(imageComments!=null){
        	...
        	for(int i=0;i<imageComments.length;i++){
        	g2d.drawString(imageComments[i].getString(), imageComments[i].x, imageComments[i].y);
        }
        }
        g2d.setFont(font);
        g2d.drawString(imageCommentTxtDsp.getString(), imageCommentTxtDsp.x, imageCommentTxtDsp.y);
        g2d.drawString(imageUserTxtDsp.getString(), imageUserTxtDsp.x, imageUserTxtDsp.y);
    //    for(int i =0;i<imageComments.length;i++){
...
/*	    	if(tmp.length()>20){
	    		tmp=new StringBuffer(tmp).insert(20, "<BR>").toString();
	    	}*/
	    	this.text = tmp;
	    }
